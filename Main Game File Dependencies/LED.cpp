...
void LED_Init(void){
    ...
    // DOE31_0 Data output enable
      GPIOA->DOE31_0 |= (1<<24);
      GPIOA->DOUTCLR31_0 = (1<<24); // LED1 off
      GPIOB->DOE31_0 |= (1<<23)|(1<<24);
      GPIOB->DOUTCLR31_0 = (1<<23)|(1<<24); // LED1 off
}
...
void LED_On(uint32_t data){
  ...
  if (data == PB23INDEX){
    setter = 1 << 23;
    GPIOB->DOUTSET31_0 |= setter;
  }

  else if (data == PB24INDEX){
    setter = 1 << 24;
    GPIOB->DOUTSET31_0 |= setter;
  }

  else if (data == PA24INDEX){
    setter = 1 << 24;
    GPIOA->DOUTSET31_0 |= setter;
  }
