...
uint32_t MyADC1_In(uint8_t memIndex){
  // Enable ADC conversions and start a conversion sequence.
  ADC1->ULLMEM.CTL0 |= 0x00000001; // enable conversions
  ADC1->ULLMEM.CTL1 |= 0x00000100; // start ADC
  volatile uint32_t delay = ADC1->ULLMEM.STATUS; // small delay for ADC startup
  
  // Wait until the conversion is complete.
  while((ADC1->ULLMEM.STATUS & 0x01) == 0x01) { }
  
  // Return the result from the desired memory register.
